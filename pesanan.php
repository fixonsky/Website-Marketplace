...
try {
    ...
    if ($id_penyedia) {
        ...
        // Pastikan jumlah parameter sesuai dengan yang di-bind
        if (!empty($params)) {
            $stmt->bind_param($paramTypes, ...$params);
        }

        if (!$stmt->execute()) {
            throw new Exception("SQL execute failed: " . $stmt->error . " Query: " . $query);
        }
        $result = $stmt->get_result();

        // Jika result masih false, handle error
        if ($result === false) {
        ...
        if (!empty($params)) {
            ...
            if (!$result) {
                throw new Exception("SQL query failed: " . $conn->error . " Query: " . $query);
            }
        }
    }
    ...
    $result = null; // Ubah dari false ke null
    $total_records = 0;
    $total_pages = 0;
}

// Debug info untuk melihat apa yang salah dengan query
if (isset($_GET['status']) && $_GET['status'] == 'telat_dikembalikan') {
    error_log("=== DEBUG FILTER TELAT DIKEMBALIKAN ===");
    error_log("ID Penyedia: " . ($id_penyedia ?: 'null'));
    error_log("Where Clause: " . ($whereClause ?? 'null'));
    error_log("Query: " . ($query ?? 'null'));
    error_log("Params: " . json_encode($params ?? []));
    error_log("Param Types: " . ($paramTypes ?? 'null'));
    error_log("Result type: " . gettype($result));
    
    if (isset($stmt) && $stmt) {
        error_log("Stmt error: " . ($stmt->error ?? 'no error'));
    }
    
    // Coba jalankan query langsung untuk debugging
    if (isset($query) && $query) {
        $debug_query = $query;
        if (isset($params) && !empty($params)) {
            // Replace parameter placeholder dengan nilai aktual untuk debugging
            foreach ($params as $param) {
                $debug_query = preg_replace('/\?/', "'" . $conn->real_escape_string($param) . "'", $debug_query, 1);
            }
        }
        error_log("Debug Query: " . $debug_query);
        
        // Jalankan query debug
        $debug_result = $conn->query($debug_query);
        if ($debug_result === false) {
            error_log("Debug Query Error: " . $conn->error);
        } else {
            error_log("Debug Query Success - Rows: " . $debug_result->num_rows);
        }
    }
}
?>
...
<head>
    ...
    .table {
        margin-bottom: 0;
        background-color: white;
        min-width: 1200px; /* Minimum width untuk table */
        width: auto;
        table-layout: auto;
    }
...
#laporanModal .modal-content {
    ...
    border-radius: 8px;
    width: 90%;
    max-width: 700px; /* Diperbesar dari 500px */
    box-shadow: 0 4px 20px rgba(0,0,0,0.3);
    position: relative;
    max-height: 90vh;
    overflow-y: auto;
...
#laporanModal .form-control:focus {
    outline: none;
    border-color: #007bff;
    box-shadow: 0 0 0 2px rgba(0,123,255,0.25);
}
...
/* Image Preview Container - DIPERBAIKI */
...
#laporanModal .image-preview-item:hover {
...
#laporanModal .btn-warning {
    background-color: #ffc107;
    color: #212529;
}

#laporanModal .btn-warning:hover {
    background-color: #e0a800;
    transform: translateY(-1px);
}

#laporanModal .btn-secondary {
    background-color: #6c757d;
    color: white;
}

#laporanModal .btn-secondary:hover {
    background-color: #5a6268;
}

#laporanModal .submit-laporan.loading {
    opacity: 0.7;
    pointer-events: none;
    position: relative;
}

#laporanModal .submit-laporan.loading::after {
    content: " (Mengirim...)";
    font-style: italic;
}
...
#laporanModal .image-preview-container::-webkit-scrollbar {
    width: 6px;
}

#laporanModal .image-preview-container::-webkit-scrollbar-track {
    background: #f1f1f1;
    border-radius: 3px;
}

#laporanModal .image-preview-container::-webkit-scrollbar-thumb {
    background: #c1c1c1;
    border-radius: 3px;
}

#laporanModal .image-preview-container::-webkit-scrollbar-thumb:hover {
    background: #a8a8a8;
}
...
.d-flex.align-items-center.gap-3 {
    ...
    .table th:nth-child(6), .table td:nth-child(6) { min-width: 100px; }
    .table th:nth-child(7), .table td:nth-child(7) { min-width: 50px; }
    .table th:nth-child(8), .table td:nth-child(8) { min-width: 40px; }
    .table th:nth-child(9), .table td:nth-child(9) { min-width: 80px; }
    .table th:nth-child(10), .table td:nth-child(10) { min-width: 80px; }
    .table th:nth-child(11), .table td:nth-child(11) { min-width: 50px; }
    .table th:nth-child(12), .table td:nth-child(12) { min-width: 80px; }
    .table th:nth-child(13), .table td:nth-child(13) { min-width: 80px; }
    .table th:nth-child(14), .table td:nth-child(14) { min-width: 120px; }
    .table th:nth-child(15), .table td:nth-child(15) { min-width: 140px; }
    ...
    /* Fix untuk zoom out yang berlebihan */
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td colspan="15" style="text-align: center; padding: 20px;">
                                    <?php 
                                    if ($result === false) {
                                        echo "Terjadi kesalahan dalam mengambil data. Silakan coba lagi.";
                                    } elseif ($result && $result->num_rows === 0) {
                                        if ($id_penyedia) {
                                            echo "Tidak ada pesanan untuk produk Anda.";
                                        } else {
                                            echo "Tidak ada data pesanan.";
                                        }
                                    } else {
                                        echo "Tidak dapat mengambil data pesanan. Silakan refresh halaman.";
                                    }
                                    ?>
                                </td>
                            </tr>
            ...
            <form id="laporanForm" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="file" id="bukti_laporan" name="bukti_laporan[]" multiple 
                           accept="image/*" class="form-control" onchange="previewMultipleImages(this)">
                    <div id="image-preview-container" class="image-preview-container"></div>
                    <small class="help-text">
                        Pilih hingga 10 gambar sebagai bukti laporan. 
                ...
                <div class="form-actions">
                    ...
                    <button type="button" onclick="submitLaporanBaru()" class="btn btn-warning submit-laporan">
                        Submit Laporan
                    </button>
                </div>
            </form>
...
function setupLaporanModal(idPesanan, idTransaksi, namaKostum, namaPenyewa) {
    ...
    // PERBAIKAN: Gunakan display flex untuk modal
