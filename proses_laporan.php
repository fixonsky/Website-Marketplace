...
try {
    ...
    // Generate ID Laporan (PINDAHKAN KE SINI)
    $randomNumbers = str_pad(mt_rand(0, 999), 3, '0', STR_PAD_LEFT);
    $idLaporan = 'LP' . $randomNumbers;

    // Cek apakah ID laporan sudah ada, jika ya generate ulang
    $checkIdQuery = "SELECT COUNT(*) as count FROM laporan WHERE id_laporan = ?";
    $stmtCheck = $conn->prepare($checkIdQuery);
    $maxAttempts = 10;
    $attempts = 0;

    while ($attempts < $maxAttempts) {
        $stmtCheck->bind_param("s", $idLaporan);
        $stmtCheck->execute();
        $resultCheck = $stmtCheck->get_result();
        $count = $resultCheck->fetch_assoc()['count'];
        
        if ($count == 0) {
            break; // ID unik ditemukan
        }
    ...
    if (isset($_FILES['bukti_laporan']) && is_array($_FILES['bukti_laporan']['name'])) {
        $fileCount = count($_FILES['bukti_laporan']['name']);
        
        // Validasi jumlah file
        if ($fileCount > $maxFiles) {
            throw new Exception("Maksimal $maxFiles gambar yang dapat diupload. Anda mencoba upload $fileCount file.");
        }
        
        foreach ($_FILES['bukti_laporan']['name'] as $key => $name) {
            $tmpName = $_FILES['bukti_laporan']['tmp_name'][$key];
            $fileSize = $_FILES['bukti_laporan']['size'][$key];
            $fileType = $_FILES['bukti_laporan']['type'][$key];
            $error = $_FILES['bukti_laporan']['error'][$key];
            
            if (!empty($name) && $error == 0) {
                // Validasi tipe file berdasarkan MIME type
                if (!in_array($fileType, $allowedTypes)) {
                    throw new Exception("File '$name' tidak didukung. Gunakan format JPG, PNG, atau GIF saja. Tipe file yang diterima: " . $fileType);
                }
                
                // Validasi ukuran file
                if ($fileSize > $maxFileSize) {
                    $fileSizeMB = round($fileSize / (1024 * 1024), 2);
                    throw new Exception("File '$name' terlalu besar ($fileSizeMB MB). Maksimal 5MB per file.");
                }
                
                // Validasi ekstensi file sebagai double-check
                $fileExtension = strtolower(pathinfo($name, PATHINFO_EXTENSION));
                if (!in_array($fileExtension, $allowedExtensions)) {
                    throw new Exception("Ekstensi file '$name' tidak diizinkan. Gunakan: " . implode(', ', $allowedExtensions));
                }
                
                // Generate nama file yang unik dengan counter
                $newFileName = 'bukti_' . $idLaporan . '_' . ($key + 1) . '_' . time() . '.' . $fileExtension;
                $uploadPath = $uploadDir . $newFileName;
                
                if (move_uploaded_file($tmpName, $uploadPath)) {
                    $uploadedFiles[] = 'uploads/laporan/' . $newFileName;
                } else {
                    throw new Exception("Gagal mengupload file '$name'. Periksa permission direktori upload.");
                }
            } elseif ($error != 0) {
                // Handle berbagai tipe error upload
                $errorMessages = [
                ...
                $errorMsg = isset($errorMessages[$error]) ? $errorMessages[$error] : "Error tidak dikenal (code: $error)";
                throw new Exception("Error upload file '$name': $errorMsg");
            }
        }
        ...
        if (!empty($uploadedFiles)) {
            ...
            // Log untuk debugging
            error_log("Successfully uploaded " . count($uploadedFiles) . " files for laporan: $idLaporan");
            error_log("Uploaded files: " . $linkBukti);
        }
    } else {
        // Jika tidak ada file yang diupload, beri peringatan tapi tidak error
        error_log("No files uploaded for laporan: $idLaporan");
    }
