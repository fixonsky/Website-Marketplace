    ...
    if ($_POST['action'] === 'pesanan_diterima' && !empty($_POST['id_pesanan'])) {
        ...
        try {
            ...
            if ($buktiFile && $buktiFile['error'] === UPLOAD_ERR_OK) {
                // OPTION A: Upload ke folder bibong/uploads/ (relatif dari root)
                ...
                if (!file_exists($uploadDirServer)) {
                    if (!mkdir($uploadDirServer, 0755, true)) {
                        throw new Exception("Gagal membuat direktori upload: " . $uploadDirServer);
                    }
                    error_log("Created directory: " . $uploadDirServer);
                }
                
                $fileExtension = strtolower(pathinfo($buktiFile['name'], PATHINFO_EXTENSION));
                ...
                $fileName = 'bukti_' . $idPesanan . '_' . time() . '.' . $fileExtension;
                ...
                if (move_uploaded_file($buktiFile['tmp_name'], $targetFile)) {
                    // PERBAIKAN PENTING: Simpan path yang bisa diakses dari root
                    $buktiPath = 'bibong/uploads/bukti_diterima/' . $fileName;
                    
                    error_log("✅ File berhasil diupload:");
                    error_log("   - Server path: " . $targetFile);
                    error_log("   - Web path: " . $buktiPath);
            ...
            $dataInsert = [
                ...
                'size' => (string)($pesananData['size'] ?? ''),
                'quantity' => (int)($pesananData['quantity'] ?? 1),
                'tanggal_mulai' => (string)($pesananData['tanggal_mulai'] ?? date('Y-m-d')),
                'tanggal_selesai' => (string)($pesananData['tanggal_selesai'] ?? date('Y-m-d')),
                'jumlah_hari' => (int)($pesananData['jumlah_hari'] ?? 1),
                'nomor_hp' => (string)($pesananData['nomor_hp'] ?? ''),
                'bukti' => (string)$buktiPath, // Pastikan ini path yang benar
                'keterangan' => (string)$catatan,
                'subtotal' => (float)($pesananData['subtotal'] ?? 0)
            ];

            // Debug sebelum insert
            error_log("Data untuk insert ke peminjaman_aktif - bukti path: " . $dataInsert['bukti']);
