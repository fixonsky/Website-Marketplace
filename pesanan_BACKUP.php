...
function submitConfirmReceived() {
    ...
    .then(data => {
        if (data.success) {
            alert('Konfirmasi berhasil! Pesanan telah ditandai sebagai diterima penyewa.');
            location.reload();
        } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    statusFilter.addEventListener('change', function() {
        ...
        tableRows.forEach(row => {
            ...
            const actionCell = row.cells[row.cells.length - 1]; // Kolom aksi (terakhir)
            
            let shouldShow = false;
            
            if (selectedStatus === 'approved') {
                // Status "Menunggu Konfirmasi" - badge approved
                shouldShow = statusBadge && statusBadge.textContent.trim().toLowerCase() === 'approved';
            } 
            else if (selectedStatus === 'accepted') {
                // Status "Menunggu Konfirmasi Diterima" - badge accepted
                shouldShow = statusBadge && statusBadge.textContent.trim().toLowerCase() === 'accepted';
            }
            else if (selectedStatus === 'pesanan_diterima_penyewa_waiting') {
                // Status "Menunggu Konfirmasi Penyewa" - Cari data yang memiliki teks "Menunggu" di kolom aksi
                const hasWaitingText = actionCell && (
                    actionCell.textContent.toLowerCase().includes('menunggu') ||
                    actionCell.innerHTML.toLowerCase().includes('menunggu')
                );
                shouldShow = hasWaitingText;
            }
            else if (selectedStatus === 'pesanan_diterima_penyewa_ready') {
                // Status "Menunggu Konfirmasi Dikembalikan" - Cari data yang memiliki tombol "Dikembalikan" di kolom aksi
                const hasReturnButton = actionCell && (
                    actionCell.textContent.toLowerCase().includes('dikembalikan') ||
                    actionCell.innerHTML.toLowerCase().includes('dikembalikan') ||
                    actionCell.innerHTML.includes('confirmReturned') ||
                    actionCell.querySelector('button')?.textContent.toLowerCase().includes('dikembalikan')
                );
                shouldShow = hasReturnButton;
            }
            
            row.style.display = shouldShow ? '' : 'none';
        });
        
        // Update counter dan nomor urut DENGAN penomoran yang benar
        updateFilterCounter();
        updateRowNumbers();
    });
});
...
function updateFilterCounter() {
    ...
    if (!counter) {
        ...
        document.querySelector('label[for="statusFilter"]').appendChild(counter);
    }
    
    if (visibleRows.length === totalRows) {
        counter.textContent = `(${totalRows} pesanan)`;
    } else {
        counter.textContent = `(${visibleRows.length} dari ${totalRows} pesanan)`;
    }
}
...
function updateRowNumbers() {
    ...
    // Hitung starting number untuk halaman ini (dari data terlama ke terbaru)
    let startingNumber = totalRecords - offset;
    
    visibleRows.forEach((row, index) => {
        const numberCell = row.cells[0]; // Kolom pertama (nomor urut)
        if (numberCell) {
            numberCell.textContent = startingNumber - index;
        }
    });
}
...
function clearFilter() {
    ...
    // Update counter dan nomor urut
    updateFilterCounter();
    updateRowNumbers();
    
    // Redirect to clear URL parameters jika ada search
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('search')) {
        window.location.href = window.location.pathname + (urlParams.has('page') ? '?page=' + urlParams.get('page') : '');
    }
}
