...
<head>
  ...
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
   ...
   .badge-success { 
    ...
    /* Adjustments for very high zoom levels */
    ...
    /* Adjustments for mobile or very high zoom */
...
<body>
    ...
    ?>
                        <tr>
                          ...
                          <td> Rp <?php echo number_format($row['harga_sewa'], 0, ',', '.'); ?> / <?php echo $row['jumlah_hari']; ?> hari
                          </td>
                          <td>
...
<script>
    ...
    function displayStokIds(stokIds) {
        ...
        stokIds.forEach((item, index) => {
            ...
            switch (item.status) {
                ...
                case 'maintenance':
                    ...
                    maintenanceCount++;
                    buttonHtml = `<button class="btn btn-xs btn-success ml-2" onclick="markSingleReady('${item.stok_id}', ${currentKostumId})" style="font-size: 10px; padding: 2px 6px;">
                                    <i class="fas fa-check"></i> Siap Disewa
                                </button>`;
                    break;
                default:
                    statusBadge = 'danger';
            ...
            html += `<div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                        <span><strong>STOK ${index + 1} - ID ${item.stok_id}</strong></span>
                        <div class="d-flex align-items-center">
        ...
        // Tampilkan tombol "Tandai Semua Siap" jika ada kostum dalam maintenance
        if (maintenanceCount > 0) {
            const readyAllButton = `<div class="mt-3 text-center">
                                        <button class="btn btn-success" onclick="markAllReady(${currentKostumId})">
                                            <i class="fas fa-check-double"></i> Tandai Semua Siap Disewa (${maintenanceCount})
                                        </button>
                                    </div>`;
            container.append(readyAllButton);
        }
    }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            ...
                            $('#stok').val(data.stok);
                            $('#outOfStock').prop('checked', data.stok == 0);
