    ...
    if ($_POST['action'] === 'pesanan_diterima' && !empty($_POST['id_pesanan'])) {
        ...
        try {
            ...
            if ($buktiFile && $buktiFile['error'] === UPLOAD_ERR_OK) {
                // Jalur server untuk mengunggah file. Ini akan menunjuk ke TUGAS_AKHIR_KESYA/uploads/bukti_diterima/
                $uploadDirServer = __DIR__ . "/../uploads/bukti_diterima/";
                
                // Jalur URL yang akan disimpan di database, relatif dari root proyek  
                $buktiPathDB = "uploads/bukti_diterima/";
                ...
                // Simpan jalur yang ramah URL ke database - INILAH YANG PENTING!
                $buktiPath = $fileName; // Hanya nama file saja, karena di peminjaman_aktif.php sudah ada path lengkap
            }
    ...
    try {
        if ($action === 'accept') {
            ...
            if ($pesananAktifData) {
                ...
                // PERBAIKAN: Set status peminjaman dengan jelas
                $statusPeminjaman = 'pesanan diterima oleh penyedia';
                ...
                error_log("DEBUG - Insert ke peminjaman_aktif:");
                error_log("Status yang akan di-set: '" . $statusPeminjaman . "'");
                error_log("ID Pesanan: " . $pesananAktifData['id_pesanan']);

                $stmtInsertAktif->bind_param("isiissississsd", 
                    $pesananAktifData['id_pesanan'],      // i
                    $pesananAktifData['id_transaksi'],    // s
                    $pesananAktifData['id_penyewa'],      // i
                    $pesananAktifData['id_penyedia'],     // i
                    $pesananAktifData['nama_kostum'],     // s
                    $pesananAktifData['size'],            // s
                    $pesananAktifData['quantity'],        // i
                    $pesananAktifData['tanggal_mulai'],   // s
                    $pesananAktifData['tanggal_selesai'], // s
                    $pesananAktifData['jumlah_hari'],     // i
                    $statusPeminjaman,                    // s - INI YANG PENTING!
                    $pesananAktifData['nomor_hp'],        // s
                    $keteranganAktif,                     // s
                    $totalHarga                           // d
                );
                ...
                $updatePeminjamanAktifQuery = "UPDATE peminjaman_aktif 
                                            SET status_peminjaman = 'penyedia konfirmasi diterima', 
                                                bukti = ?
                                            WHERE id_pesanan = ?";
                $stmtUpdatePeminjamanAktif = $conn->prepare($updatePeminjamanAktifQuery);
                $stmtUpdatePeminjamanAktif->bind_param("si", $buktiPath, $idPesanan);
                
                if (!$stmtUpdatePeminjamanAktif->execute()) {
                    error_log("Gagal mengupdate peminjaman_aktif untuk pesanan {$idPesanan}: " . $stmtUpdatePeminjamanAktif->error);
                    // Tidak throw exception karena ini tidak kritis, pesanan sudah terupdate
                } else {
                    error_log("✅ Berhasil update peminjaman_aktif untuk pesanan {$idPesanan} dengan status 'penyedia konfirmasi diterima'");
            }

                // Verifikasi data yang ter-insert
                $verifyQuery = "SELECT status_peminjaman FROM peminjaman_aktif WHERE id_pesanan = ? ORDER BY id_peminjaman_aktif DESC LIMIT 1";
                $stmtVerify = $conn->prepare($verifyQuery);
                $stmtVerify->bind_param("i", $pesananAktifData['id_pesanan']);
                $stmtVerify->execute();
                $verifyResult = $stmtVerify->get_result();
                $verifyData = $verifyResult->fetch_assoc();
                
                error_log("✅ Status yang ter-insert di database: '" . ($verifyData['status_peminjaman'] ?? 'NULL') . "'");
                error_log("✅ Data berhasil diinsert ke peminjaman_aktif untuk pesanan {$idPesanan} dengan status 'pesanan diterima oleh penyedia'");
            }
            ...
            try {
                ...
                $logMessage = "Pesanan {$idPesanan} dikonfirmasi diterima penyewa dengan bukti foto: {$filePath}";
                if (!empty($catatan)) {
                    $logMessage .= ". Catatan: {$catatan}";
                }
                error_log($logMessage);

                $insertPeminjamanAktif = "INSERT INTO peminjaman_aktif 
                    (id_pesanan, id_transaksi, id_penyewa, id_penyedia, nama_kostum, size, quantity, 
                     tanggal_mulai, tanggal_selesai, jumlah_hari, status_peminjaman, nomor_hp, bukti, keterangan) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'sedang_berjalan', ?, ?, ?)";

                $stmtPeminjamanAktif = $conn->prepare($insertPeminjamanAktif);

                // Ambil data dari pesanan untuk insert ke peminjaman_aktif
                $getPesananForPeminjaman = $conn->prepare("
                    SELECT p.*, t.id_user as id_penyewa 
                    FROM pesanan p 
                    JOIN transaksi t ON p.id_transaksi = t.id_transaksi 
                    WHERE p.id_pesanan = ?
                ");
                $getPesananForPeminjaman->bind_param("i", $idPesanan);
                $getPesananForPeminjaman->execute();
                $pesananData = $getPesananForPeminjaman->get_result()->fetch_assoc();

                if ($pesananData) {
                    $keterangan = "Pesanan dikonfirmasi diterima penyewa oleh penyedia";
                    if (!empty($catatan)) {
                        $keterangan .= ". Catatan: " . $catatan;
                    }
                    
                    $stmtPeminjamanAktif->bind_param(
                        "isissiisssis",
                        $pesananData['id_pesanan'],
                        $pesananData['id_transaksi'], 
                        $pesananData['id_penyewa'],
                        ...
                        $buktiForDB, // bukti foto
                        $keterangan
                    );
                    
                    if (!$stmtPeminjamanAktif->execute()) {
                        error_log("Gagal insert ke peminjaman_aktif: " . $stmtPeminjamanAktif->error);
                    } else {
                        error_log("✅ Data berhasil diinsert ke peminjaman_aktif untuk pesanan {$idPesanan}");
                    }
                }
            ...
            $updateTransaksiItemsQuery = "UPDATE transaksi_items 
                                        SET status_item = 'costume_returned',
                                            updated_at = CURRENT_TIMESTAMP
                                        WHERE id_transaksi = ? AND id_produk = ?";
            $stmtUpdateItems = $conn->prepare($updateTransaksiItemsQuery);
            $stmtUpdateItems->bind_param("si", $pesanan['id_transaksi'], $pesanan['id_produk']);
            ...
            $insertProviderReturnConfirmation = "INSERT INTO provider_return_confirmations (id_pesanan, id_transaksi, id_penyedia) 
                                            VALUES (?, ?, ?)
                                            ON DUPLICATE KEY UPDATE confirmed_at = CURRENT_TIMESTAMP";
            $stmtReturnConfirm = $conn->prepare($insertProviderReturnConfirmation);
            $stmtReturnConfirm->bind_param("isi", $idPesanan, $pesanan['id_transaksi'], $pesanan['id_penyedia']);
            $stmtReturnConfirm->execute();
