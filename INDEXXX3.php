...
$whereClause = "WHERE f.id_user = ?";
$params = [$iduser];
$paramTypes = "i";

if (!empty($search)) {
    $whereClause .= " AND (f.judul_post LIKE ? OR f.kategori LIKE ? OR f.series LIKE ? OR f.karakter LIKE ? OR f.gender LIKE ?)";
    $searchParam = "%$search%";
    for ($i = 0; $i < 5; $i++) {
        $params[] = $searchParam;
        $paramTypes .= "s";
...
switch ($sort) {
    case 'oldest':
        ...
        break;
    case 'name_asc':
        $orderClause .= "f.judul_post ASC";
        break;
    case 'name_desc':
        $orderClause .= "f.judul_post DESC";
        break;
    case 'price_asc':
        $orderClause .= "f.harga_sewa ASC";
        break;
    case 'price_desc':
        $orderClause .= "f.harga_sewa DESC";
        break;
    default: // newest
        $orderClause .= "f.id DESC";
        break;
}

$count_query = "SELECT COUNT(*) as total FROM form_katalog f $whereClause";
...
.modal-overlay {
    ...
    z-index: 9999; /* PERBAIKAN: Tetap di 9999, lebih rendah dari booking modal */
    justify-content: center;
    align-items: flex-start;
    padding-top: 50px;
    overflow-y: auto;
...
.modal-content {
    ...
    z-index: 10000; /* PERBAIKAN: Tambahkan z-index untuk content */
}
...
.modal-footer {
    ...
    @media (max-width: 576px) {
        ...
        .modal-overlay {
            padding-top: 20px; /* Kurangi padding di mobile */
            align-items: flex-start;
        }
    ...
    @media (min-height: 800px) {
        .modal-overlay {
            align-items: center; /* Kembali ke tengah untuk layar tinggi */
            padding-top: 0;
        }
...
#bookingModal {
    ...
    bottom: 0;
    background-color: rgba(0, 0, 0, 0.5);
    z-index: 10000; /* PERBAIKAN: Tingkatkan z-index menjadi lebih tinggi */
    justify-content: center;
    align-items: center;
}
...
#bookingModal .modal-content {
    ...
    max-width: 90%;
    position: relative;
    z-index: 10001; /* PERBAIKAN: Pastikan content juga memiliki z-index tinggi */
}
...
<body>
    ...
    <div class="container">
        ...
        <main class="col-md-20 ml-sm-auto col-lg-30 px-4 content" role="main">
            ...
            <div class="alert alert-success alert-dismissible fade show" role="alert" style="margin: 10px 0 20px 0;">
                <i class="fas fa-check-circle"></i> <strong>Berhasil!</strong> Data kostum telah disimpan.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
            <script>
                // Auto remove parameter from URL setelah alert ditutup
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if (alertElement) {
                        ...
                        // Auto remove parameter after 5 seconds jika alert tidak ditutup manual
                        setTimeout(function() {
                            const url = new URL(window.location);
                            url.searchParams.delete('saved');
                            window.history.replaceState({}, '', url.toString());
                ...
                <div class="d-flex align-items-center">
                    <form method="GET" class="d-flex align-items-center">
                        ...
                        <div class="input-group">
                            <input name="search" class="form-control" placeholder="Cari kostum, kategori, series..." type="text" 
                                    ...
                                    <form id="updateStockForm">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="booking-btn-container">
                                                <!-- PERBAIKAN: Tambahkan type="button" untuk mencegah form submission -->
                                                <button type="button" id="bookingBtn" class="btn">Booking</button>
                                            </div>
                                        </div>
            ...
            <div class="text-muted">
                <?php if (!empty($search)): ?>
                    <small>
                        ...
                        <?= $total_records ?> kostum ditemukan
                    </small>
                <?php else: ?>
                    <small>
                        Menampilkan <?= min($records_per_page, $result->num_rows) ?> dari <?= $total_records ?> data
                    </small>
                <?php endif; ?>
            </div>
            
            <?php if (!empty($search) || isset($_GET['sort'])): ?>
                <div>
...
document.addEventListener('keydown', function(event) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function initUpdateCalendar(kostumId) {
            ...
            // Ambil data jumlah hari sewa dari tabel
            ...
            console.log('Durasi sewa:', window.rentalDuration, 'hari');
            ...
            calendarInstance = new FullCalendar.Calendar(calendarEl, {
                ...
                dateClick: function(info) {
                    ...
                    updateSelectedDatesInfo();
                    $('.booking-btn-container').show(); // PASTIKAN tombol booking muncul
                }
            });
            ...
            // PERBAIKAN: Setup event handler untuk toggle date selection
            $('#toggleDateSelection').off('click').on('click', function() {
                ...
                if (isSelectingDates) {
                    $('.booking-btn-container').show(); // Tampilkan tombol booking saat mode seleksi aktif
                } else {
                    $('.booking-btn-container').hide(); // Sembunyikan tombol booking saat mode seleksi non-aktif
                    // Simpan otomatis untuk pemilik jika ada tanggal yang dipilih
            ...
            // PERBAIKAN KRITIS: Setup event handler untuk tombol booking - DENGAN PREVENT DEFAULT
            $('#bookingBtn').off('click').on('click', function(e) {
                e.preventDefault(); // Mencegah form submission
                e.stopPropagation(); // Mencegah event bubbling
                
                console.log('Tombol Booking diklik, selectedDates:', selectedDates);
                if (selectedDates.length > 0) {
                    showBookingModal();
                } else {
                    alert('Silakan pilih tanggal terlebih dahulu dengan mengklik "Pilih Tanggal" dan memilih tanggal di kalender');
                }
            });
        }
        ...
        function showBookingModal() {
            ...
            console.log('Menampilkan modal booking dengan dates:', selectedDates);

            // PERBAIKAN: Sembunyikan dulu modal update stok sementara
            $('#updateStockModal').hide();
            
            // PERBAIKAN: Pastikan modal booking benar-benar muncul di atas
            $('#bookingModal').css({
                'display': 'flex',
                'z-index': '10000'
            }).show();
            
            // Setup ulang event handler untuk confirm booking
            $('#confirmBooking').off('click').on('click', function(e) {
                e.preventDefault();
                confirmBooking();
            });
            ...
            // Reset nama pemesan dan fokus
            $('#bookingName').val('');
            $('#bookingName').focus();
            
            console.log('Modal booking berhasil ditampilkan');
        }
        ...
        function saveSelectedDates(kostumId) {
            ...
            console.log('Data pemilik yang akan dikirim:', bookingData);

            // PERBAIKAN: Tetap gunakan JSON untuk konsistensi
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response && response.status === 'success') {
                        ...
                        // Beri feedback
                        showToast('Tanggal berhasil dibooking untuk pemeliharaan', 'success');
                    } else {
                        console.error('Error dari server:', response?.message);
                        showToast('Gagal menyimpan booking: ' + (response?.message || 'Unknown error'), 'error');
                    }
                },
                error: function(xhr, status, error) {
                    console.error('Error AJAX:', error, xhr.responseText);
                    $('#bookingBtn').prop('disabled', false).html('Booking');
                    showToast('Terjadi kesalahan saat menyimpan booking', 'error');
                }
            });
        }
        ...
        function confirmBooking() {
            ...
            console.log('Data booking yang akan dikirim:', bookingData);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response && response.status === 'success') {
                        ...
                        // PERBAIKAN: Tampilkan kembali modal update stok
                        $('#updateStockModal').show();
                        
                        // Hapus data temporary
                        localStorage.removeItem('tempSelectedDates_' + currentKostumId);
                        
                        // Refresh calendar untuk menampilkan dates yang baru dibooking
                        if (calendarInstance) {
                            calendarInstance.refetchEvents();
                        }
                        ...
                        const errorMsg = response?.message || 'Gagal menyimpan booking';
                        alert('Error: ' + errorMsg);
                        
                        // PERBAIKAN: Tampilkan kembali modal update stok meskipun error
                        $('#updateStockModal').show();
                    }
                ...
                error: function(xhr, status, error) {
                    ...
                    try {
                        ...
                        errorMsg = 'Terjadi kesalahan server. Periksa console untuk detail.';
                    }
                    alert('Error: ' + errorMsg);
                    $('#confirmBooking').prop('disabled', false).html('Konfirmasi');
        ...
        $('#closeBookingModal, #cancelBooking').off('click').on('click', function(e) {
            e.preventDefault();
            $('#bookingModal').hide();
            
            // PERBAIKAN: Tampilkan kembali modal update stok ketika booking modal ditutup
            $('#updateStockModal').show();
        });
        ...
        function showUpdateStockModal() {
            ...
            isSelectingDates = false;
            updateSelectedDatesInfo();
            $('.booking-btn-container').hide(); // Pastikan tombol booking tersembunyi saat modal pertama dibuka
            
            console.log('Modal update stock ditampilkan');
        }
        ...
        function hideUpdateStockModal() {
            ...
            // Hapus data temporary dari localStorage
            if (currentKostumId) {
                localStorage.removeItem('tempSelectedDates_' + currentKostumId);
            }
        }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        // Load existing stock IDs
                        loadStokIds(kostumId);
                        
                        $('#updateStockForm [name="id"]').val(data.id);
                        
                        // PERBAIKAN: Initialize calendar dan setup event handlers
                        setTimeout(() => {
                            initUpdateCalendar(kostumId);
                        }, 100);
                        
                        // Tampilkan modal setelah semua data dimuat
        ...
        $('#updateStockForm').on('submit', function(e) {
            ...
            console.log('Submitting form dengan kostumId:', kostumId, 'stok:', stok);
            ...
            // PERBAIKAN: Gunakan data biasa dengan error handling yang lebih baik
            $.ajax({
                url: 'update_stok.php',
                type: 'POST',
                data: {
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        const stokElement = $(`button.update-btn[data-id="${kostumId}"]`).closest('tr').find('.stok-value');
                        if (stokElement.length) {
                            stokElement.text(stok);
                        }
                        ...
                        const errorMsg = response.message || 'Terjadi kesalahan yang tidak diketahui';
                        showToast('Error: ' + errorMsg, 'error');
                        console.error('Update stok error:', response.message);
                    }
        ...
        $(document).on('keydown', function(e) {
            if (e.key === 'Escape' && $('#updateStockModal').is(':visible')) {
                hideUpdateStockModal();
            }
        });
    });
...
<?php if (isset($_GET['saved']) && $_GET['saved'] == '1'): ?>
<script>
// Backup dengan SweetAlert jika diperlukan
document.addEventListener('DOMContentLoaded', function() {
    // Cek apakah Bootstrap alert sudah ditampilkan
    const bootstrapAlert = document.querySelector('.alert-success');
    if (!bootstrapAlert || bootstrapAlert.style.display === 'none') {
        // Jika Bootstrap alert tidak terlihat, gunakan SweetAlert
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                title: 'Berhasil!',
                text: 'Data kostum telah disimpan',
                icon: 'success',
